...
export async function GET(request: NextRequest) {
  try {
    ...
    // Aggregate properties by listing type (rent vs sale)
    const listingTypeStats = await Property.aggregate([
      {
        $match: {
          deletionStatus: { $ne: 'deleted' }, // Only count active properties
          listingType: { $exists: true, $nin: [null, '', undefined] } // Filter out null/empty listing types
        }
      },
      {
        $group: {
    ...
    const chartData = listingTypeStats.map(stat => ({
      name: stat._id,
      value: stat.count,
    ...
    return NextResponse.json({
      ...
      data: {
        ...
        summary: {
          totalProperties,
          totalValue,
          totalListingTypes: chartData.length
        }
      }
    })

  } catch (error) {
    console.error('Error fetching listing type stats:', error)
    return NextResponse.json(
